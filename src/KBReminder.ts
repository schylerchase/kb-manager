...
export default class KBReminder {
  constructor(private app: App, private settings: KBManagerSettings) {}

  addCommands(plugin: Plugin): void {
    plugin.addCommand({
      id: 'kb-manager-create-update-reminder',
      name: 'KB Manager: Create KB update reminder',
      callback: () => { this.createUpdateReminder('').catch(err => this.reportError(err)); },
    });
  }
  ...
  async createUpdateReminder(scopePath: string): Promise<void> {
    ...
    if (await this.tryQuickReminder(reminder)) {
      new Notice(`KB Manager: reminder set for ${new Date(dueAt).toLocaleDateString()}`);
      return;
    }
    await this.writeFallbackTask(scopePath, dueAt);
    new Notice(`KB Manager: review task added to ${this.settings.kbReviewTaskPath}`);
  }
  ...
  private async tryQuickReminder(reminder: unknown): Promise<boolean> {
    ...
    if (!quickReminder?.store?.add || !quickReminder.scheduler?.schedule) return false;
  ...
  private getQuickReminder(): QuickReminder | null {
    ...
    return plugin && typeof plugin === 'object' ? plugin as QuickReminder : null;
  }

  private async writeFallbackTask(scopePath: string, dueAt: number): Promise<void> {
    const path = normalizePath(this.settings.kbReviewTaskPath || 'KB Updates.md');
